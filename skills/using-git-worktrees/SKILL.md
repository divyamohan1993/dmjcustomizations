...
| Two tasks want one file | Never in parallel. Sequence them, or one produces a patch the lead applies after the other lands |
...
- Ownership boundaries stated in the spawn prompt, never assumed. A teammate editing outside its set = stop-and-fix, not a merge conflict to untangle later.
- Baseline before building: install deps, run the suite. Baseline failures get reported before new work starts, so new breakage stays distinguishable.
- Stray worktree found in a repo (legacy or hand-made): report it to the user; remove only on their word (`git worktree remove` + `prune`), never adopt it for new work.
