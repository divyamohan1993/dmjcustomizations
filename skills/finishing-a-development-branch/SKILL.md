...
`GIT_DIR != GIT_COMMON` = a worktree exists (someone or something created one; worktrees are banned by user law, so surface it to the user rather than adopting it). Detached HEAD removes the local-merge option.
