...
A reviewer (human or a review-lens teammate) can be wrong, lack context, or hallucinate a bug. Before changing code, confirm the claim against reality: does the cited line actually do what the finding says? Does the suggestion break existing behavior, or any platform/version still supported? Is there a reason the current code is the way it is? Unverified compliance is how a confident-but-wrong finding becomes your bug.

Cannot verify it (missing context, external dependency)? Say so: "I cannot verify this without X. Investigate, ask, or proceed?" Do not proceed on faith.

## No performative agreement

Forbidden, this is a standing instruction, not a style preference: "You're absolutely right," "Great point," "Excellent feedback," any thanks. They add nothing and signal compliance over thought.
...
Independent items can be assessed concurrently (verify several at once, or fan out to teammates for a large batch via `dmjcustomizations:dispatching-parallel-teams`). But items often relate: if any is unclear, clarify ALL unclear items before implementing ANY, since partial understanding yields wrong fixes. Then implement blocking items (breaks, security) first, simple fixes (typos, imports) next, complex (refactor, logic) last, testing each with no regressions.

## YAGNI check

Reviewer says "implement this properly"? Grep for actual usage first. Nothing calls it: "This isn't called anywhere, remove it (YAGNI)?" It is used: implement it properly.
...
A background agent verifies every item against the code, implements what survives, and records in the final report what it changed, what it pushed back on with reasoning, and what it could not verify. Park only genuine architectural conflicts the user owns.
