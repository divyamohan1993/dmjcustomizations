...
for s in $STACKS; do case "$s" in
  ...
  shell)
    setlane FMT shell "git diff --check && git diff --cached --check && git diff HEAD~1 HEAD --check"
    setlane LINT shell "shellcheck \$(repo_files | grep -E '\\.sh$')"
    setlane SAST shell "shellcheck \$(repo_files | grep -E '\\.sh$')"
    setlane UNIT shell "bats tests"
    setlane ACCEPT shell "bats tests"
...
# ---------------------------------------------------- lane command manifest --
{
  ...
  echo '# shellcheck disable=SC2034 # sourced by qgate.sh; lane assignments are consumed there.'
...
# The following single-quoted value intentionally preserves literal shell
# expansions for the generated qgate.sh runner.
