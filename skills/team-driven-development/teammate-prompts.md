...
Three team-driven-development role contracts; assemble each prompt from its rows. Every field LITERAL: paste the task text, the report, the diff range. A pointer resolves to nothing on the other side (a teammate inherits NONE of the lead's conversation: `team-mechanics.md`). Reviewers run FRESH context, never the implementer reviewing itself.
