...
- **Whole-library terse register** (user order): all 44 skill markdown files rewritten telegraphic (fragments, `=`, `->`, articles dropped) by four parallel rewriters under disjoint file ownership in the main checkout, style-only. Measured shrink 7% aggregate (2-17% per file) against an honest ceiling test: the corpus was already two compression passes deep, and every remaining sentence is a law, number, enum, or the rationale that makes a mandate stick; 40%+ was reachable only by deleting propositions, which the preservation brief forbade. Verified per file: protection-line ratchet flat, all table rows, `dmj:` pointers, numbers-with-units, citations, and fences byte-identical, descriptions untouched, both gates green. The two crypto files keep sentence case so mandate leads keep their visual weight.
- **Frameworks in force, actively, for everything AI writes in target repos** (user order): the generated gate's ASD-STE100 lane is now blocking by default (`STE_ENFORCE=1`, per-repo `STE_ALLOWLIST`, dated waivers for legacy human docs, never a disabled lane, UNAVAILABLE on empty scope); EARS gains its authoring half (agents WRITE requirements as EARS patterns under `## Requirements` in writing-plans and brainstorming specs; the lane verifies what they wrote); the binding line lands in enforcing-quality-gates; equipping-projects' prose row wires EARS + STE + humanizer onto AI-authored docs; gate-matrix's config fence documents the new defaults. README and CLAUDE.md declare the roster: EARS, ASD-STE100, Gherkin, OWASP ASVS L2, WCAG 2.2 AAA, Keep a Changelog, conventional commits, DPDP + GDPR.
- **Agentic-framework framing** (user order): README hero and orchestrating-products open with what this is: a research-grade, production-deployable agentic software engineering framework; skills = policy, hooks = enforcement, generated gates = verification, teams = execution.

## [5.0.0] - 2026-07-29

Third user directive batch, the terse-register doctrine, folded pre-release with then-vs-now corrections: **worktrees banned by user law** (recorded reason: work lost under a switched-out branch; using-git-worktrees rewritten as the isolation policy: main checkout, strict file-ownership boundaries, sequencing or patch-apply for overlap, temp-clone spikes; 14 files swept); **every spawn `opus[1m]`** (the two-tier menu retired; single judgement tier everywhere); **AI-tell lexicon grown** by the user's banned list (`streamline`, `innovative`, `empower`, `holistic`, `synergy`, `paradigm`, `comprehensive`, and kin; repo prose verified clean under the expanded gate); floors raised where the doctrine names them: WCAG 2.2 AAA with the equal-userbase law, baseline device (slow phone, bad internet, small town), MLP-not-MVP with onboarding-under-30s and errors-as-product-moments (crafting-experiences), two-audience observability with SIEM-grade super-admin view and p50/p95/p99 (observing-production), deploy-is-not-release with flags, kill switch, canary, sub-60s rollback (shipping-to-production), API v2 row (REST-or-GraphQL never mixed, /v1/, cursor pagination, idempotency keys, nested error shape, stale-docs-fail-build; equipping-projects), DPDP + India data residency (defending-in-depth), perf/offline/arch shapes (stack-defaults). The stale TeamCreate/team_name mechanism in the draft was translated to the live orchestrator law rather than adopted. CLAUDE.md rewritten in the draft's telegraphic register: 6,136 chars, all 23 law probes green.

Version 5 by user order: the Claude 5 family optimizations land as a major. Two user directives folded in at the close: **the orchestrator law** (the main thread always orchestrates and never labors; ALL substantive work runs in background agents with their own context windows; only conclusions enter the main thread; delegation is the default even for a single task) now lives in using-dmj (law), dispatching-parallel-teams (mechanics, context named as the defended budget), and CLAUDE.md principle 6 (pointer); and **governance shifted to dmjcustomizations** (CLAUDE.md carries identity + laws only, 16,751 to 5,917 chars total across both slims; API error schema moved to equipping-projects, container hardening and the super-admin panel to shipping-to-production's artifact table).

The Claude 5 "then and now" pass: five fresh-context sweeps over all 74 tracked files plus the 8 global context files, four adversarial lens reviews on the design, user-approved, executed as a 12-commit series with the security work gated. Full per-file accounting: docs/dmj/specs/2026-07-29-claude5-then-now-pass-ledger.md; design: docs/dmj/specs/2026-07-29-claude5-then-now-pass-design.md. Globals: CLAUDE.md 16,751 to 9,813 chars with every imperative kept (~1.7K tokens saved per session); ship-checklist rewritten as a gate map after its protections were homed in skills; two reference files created (stack-defaults, super-admin). Late one-home trims: enforcing-performance-budgets (stack-choice row + flag), shipping-to-production (2 rows, 3 flags), humanizing-output (dash set and commit-msg claims now point at code). Standing user decisions: release gate kept permanently, DA ritual compressed, RTK import kept shrunk. install-gate polyglot fixture evidence: `.qgate-lanes.sh` now emits both `L_UNIT_node=npm exec vitest run` and `L_UNIT_shell=bats tests`.

### Removed

- Rationalization tables and red-flag lines that restated a bar, step, or rule already stated above them in the same file, per the writing-skills one-home test, in: test-driven-development (4 of 7 rows; kept too-simple, tests-after, sunk-cost), verification-before-completion (whole table; the guard sentence moved into "Verified stays verified"), receiving-code-review (the 6-step ritual block; verify-before-implement carries the law), observing-production (table plus one duplicate flag), crafting-experiences (table and flags; the adapt-from-signals rule moved into the Burden bar, the deadline line kept as prose), art-directing (attractor section; the detection sentence moved into Gate 0), selling-the-vision (table; the deadline clause moved into the Tagline bar), researching-deeply (flag section; every flag restated a stated rule), equipping-projects (table and flags; first-unscanned-commit moved into the gate), exploring-codebases (table and 2 of 4 flags), tracing-codebases (4 of 5 flags; presumably/probably kept, folded into Synthesize), writing-plans (flag section), executing-plans (flag section), team-driven-development (3 of 6 flags), dispatching-parallel-teams (6 of 9 flags). Every surviving law is unchanged; deletions are restatements only. Authoring-time probe for this batch: four fresh-context adversarial lens reviews (pre-mortem, YAGNI, ambiguity, security) attacked each cut before merge, and the per-row test plus rescued sentences are recorded in the design spec.
- testing-anti-patterns: two TypeScript vignettes whose point the smell/fix table already carries (test-only-method, mock-without-understanding); the incomplete-mock example stays. debugging-techniques: the waitFor implementation replaced by its spec (the model writes the code from the spec; the BAD/GOOD pair and the mistakes list stay).
...
- **Judgement-era rewrites across the library** (the "then and now" pass): systematic-debugging's self-contradictory attempt arithmetic fixed to a hard two-consecutive-failures stop with the reason the count exists; writing-skills' Iron Law rescoped by property (floors, gates, thresholds, descriptions need an authoring-time probe recorded in commit message and CHANGELOG; mechanical edits do not) and its headless/parallel section mandate replaced by a library default in using-dmj that skills override only on deviation; bright-line doctrine in testing-skills-with-teams scoped to irreversible/expensive classes with the anti-sycophancy guard kept; evolving-skills' retirement evidence re-pointed from deleted batteries to authoring-time pressure probes; orchestrating-products' unbacked 20-runs/95% number replaced by the no-number-without-a-producer principle; observing-production and requesting-code-review forced counts became principles; landing-sessions and harnessing-claude rewritten for auto-memory (curation over hand-authoring); equipping-projects points at native /init and names all four gate files; using-git-worktrees drops the EnterWorktree hedge; writing-plans accepts a committed artifact (failing test, fixture, mock) as a step's spec; teammate tier literals consolidated to dispatching-parallel-teams with role names elsewhere.
- **Single-home dedup completed**: the trivial-change clause list now has one in-repo home (enforcing-quality-gates, stated in full deliberately for CLAUDE.md-less surfaces) with brainstorming pointing; the teams env gate lives only in team-mechanics.md; team-mechanics keeps the docs-sourced sizing numbers while granularity rationale lives in writing-plans; teammate-prompts.md rebuilt as field-table contracts (interface over paste-block) keeping the status enum and the too-hard bar; review-lens names the judgement tier without quoting the alias; brainstorming's rationalization table keeps only its two definitional rows and its headless block keeps only the never-deadlock delta; pure-default Headless blocks removed from tracing-codebases, exploring-codebases, and landing-sessions (the default binds from using-dmj).
- Stale "team-based workflows"/"agent-teams"/"agent teams" vocabulary replaced across plugin.json, marketplace.json, README.md, and export-claude-ai.mjs ("teammate fan-out", "delegation tools"); release.sh now rebuilds the claude.ai export after every release so dist/ cannot ship deleted text.
...
- **Deep optimization sweep for the Claude 5 family** (user-ordered), applying the five context-engineering principles (judgement over rules, interfaces over examples, progressive disclosure, one home per rule, references over restatement) across all 32 skills via a 4-agent team with adversarial cross-checks and an independent reviewer. **Zero law retirements**: across ~74 traced cuts, every removal was a duplicate home, justification prose, or a form violation, each with a named in-file destination or a written contract branch justifying it. All 32 frontmatter descriptions byte-identical, preserving the routing-gauntlet certification.
- using-dmj router rebuilt as six labelled blocks, 2036 to 1913 chars on the always-injected surface; every floor kept; the trivial-change pointer now names the in-repo home (enforcing-quality-gates) so the gate stays reachable with CLAUDE.md absent.
- quantum-durable-crypto.md deduplicated against defending-in-depth, 1471 to 917 words (-38%): verbatim profile tables, override rules, and "What not to do" copies died; unique depth stays (hybrid combiner, envelope, key hierarchy, migration order, libraries, AEGIS-vs-GCM rationale); content existing only there carried up into SKILL.md before deletion. The TLS-1.3-is-covered trap moved INTO the SKILL.md rationalization table, a strict improvement for battery 4A since the harness pastes SKILL.md only.
- Spawn contract consolidated to its mechanism owner (dispatching-parallel-teams); five restatement sites reduced to pointers keeping only skill-specific assignments, all four depended elements verified surviving. teammate-prompts.md rebuilt on the documented no-inherited-history fact. review-lens.md adopts the two-tier menu (`opus[1m]` judgement) in the file the 2.20.0 pass missed; its Experience lens anchors to the crafting-experiences non-negotiables table.
- humanizing-output points at its own code: inline AI-tell lexicon replaced by the `AI_TELLS` set in humanize-guard.mjs plus 8 inline exemplars; three prose sections became one file-to-spec table; the dashes-always vs tells-under-gate distinction intact.
- Rationalization and red-flag surfaces deduplicated under the completed written contract (below): 15 hollow flags cut in the experience cluster (bar text negated lines below the bar), 5 domain rows promoted-then-cut into the artifacts their battery PASS criteria cite, orchestrating-products' table deleted (19A's PASS names the token-economy section). Genuine stop lists untouched: audits across two clusters (45 + 11 flags) forced zero deletions of symptom-naming triggers, and the reviewer confirmed asymmetric judgement (executing-plans cut a flag its Step 1 states; team-driven-development kept the identical flag, having no equivalent step).

### Added

- **dispatching-parallel-teams/team-mechanics.md**: agent-teams harness mechanics from the official docs (user-supplied), reviewer-verified against the live page with zero invented or misstated mechanics: env gate `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`, inheritance table, task-list internals, TeammateIdle/TaskCreated/TaskCompleted exit-2 hooks, ignored role-definition frontmatter, limitations, state locations, docs URL. Floors stay inline in SKILL.md: env gate with subagent fallback, no nested teams, no context inheritance, permissions-do-not-travel, disjoint file ownership. harnessing-claude gains team gate hooks, Skill-tool, and teammate plan-approval rows.
- **writing-skills REFACTOR lookup completed to three branches** (the pass stress-tested its own contract): a test PASS citing a section makes that section the home; a test or CHANGELOG recording the row keeps the row; a no-scenario row is cuttable iff it restates a bar, step, or rule in the same file, and a row naming an uncovered pressure stays. Plus the red-flag clause: a detection trigger earns its place by naming the symptom or preceding thought; one repeating a bar's words is the duplicate, and two flags covering one law merge. Deliberately a content test that can fail: a kind-based exemption was proposed, tested against selling-the-vision's five bar-negations, and withdrawn by its own author. best-practices.md gains References-over-restatement (code is the spec, rubrics are references, a spec can be a rich artifact).
- **evolving-skills retirement law**: a law retires only after re-running the test that motivated it; an assertion is not evidence, a passing self-check is not the test, a no-test law needs a fresh RED run, a user-ordered law retires only on the user's word. Capture became a 5-field schema. Human-merge floor strengthened: never edit an installed skill mid-session (battery 13A's exact FAIL option). Threat model byte-identical.
- **Five battery PASS criteria moved from test into text** (previously derivable only by inference; three claimed, two found by the reviewer): 10C's action-count rule into the crafting-experiences Burden bar, 10D's adapt-from-signals answer into the More-options row, 12A's category-study criterion into art-directing Gate 0 (now also negating its FAIL option), 4A's TLS rebuttal into defending-in-depth's rationalization table, 14C's three-part cost disclosure into shipping-to-production (previously requiring CLAUDE.md).
- exploring-codebases opens with the reciprocal boundary pointer to tracing-codebases.

### Fixed

- **Ciphertext envelope geometry forbade the MAX profile**: `[nonce:12][tag:16]` is AES-256-GCM geometry; AEGIS-256 needs a 32-byte nonce and the mandated 256-bit tag is 32 bytes, so an implementer following the spec literally could never store data at the strongest at-rest profile. Now length-by-`alg_id` with both suites' concrete values named; the old geometry greps empty repo-wide.
- brainstorming red flag "questions one at a time instead of batched" had forbidden what flow step 1 explicitly allows since 2.19.0 added the interview mode. Contradiction removed.
- dispatching-parallel-teams' flowchart replacement briefly routed overlapping-file work as both fan-out-with-worktree and stay-in-one-session; caught by an advisor pass and fixed pre-review.
- scripts/export-claude-ai.mjs skipped skill subdirectories, so claude.ai zips shipped humanize-guard.mjs but not hooks/pre-push; the copy walk now recurses (the zip step always did), verified live across all 32 exports.
- Post-review contract repairs (reviewer findings F1, F4, applied by the lead as ruling-alignment): the one-home discriminator broadened to "a floor, step, or rule stated above it" with the same-kind merge case added, matching how all three clusters actually applied it; the body norm reworded to "the excess is usually depth" so the honest-overage case is licensed by the text rather than by adjudication.

### Audit notes

- **Adversarial review verdict: SHIP.** Gates on the final state: validate.js PASS (32 skills), humanize-guard clean on all 44 changed files, zero unicode dashes, frontmatter grep empty, all 32 `dmj:` refs and every backticked reference file resolve, behavioral-test.sh 9/9. Battery walk: every cited scenario's PASS criterion derivable from the edited SKILL.md alone; nothing regressed. Crypto floor: all 12 non-negotiables verbatim.
- Adjudication: executing-plans (per-task plus per-wave gates) and team-driven-development (single task gate split into ordered stages) stay distinct; they gate different objects at different granularities, are never co-loaded, and unifying would reintroduce the serialization the wave model exists to avoid.
- Body-length norm measured across all 32 skills (two independent measurements, agreeing exactly): 26 comply, 6 run over, and every one of the 6 owns a law set (crypto parameters 927, the delegation contract 851, the skill-format contract 850, the gate matrix 757, brainstorming 554, team-driven-development 505). Five of the six had already split to sibling files and still run over, and five under-norm files also carry siblings, ruling out circularity: the overage is law that must stay inline, not undisclosed depth. Splitting recovers less than it appears to (one measured case returned 26 words against an estimated 150). brainstorming (554, zero siblings) is the library's one untested split candidate, deferred with a measure-first caution.
- Unverified-numbers pattern, named for the learning queue: four confidently stated estimates this pass, every one corrected by someone other than its author, every correction from measuring rather than reasoning. Candidate skill-learning at landing, pending user confirmation per the capture law.
- Queued for a battery-capable pass: reviewer findings F2 (stewarding-data's "Small table, ALTER inline" excuse no longer named; the unconditional rule holds), F3 (selling-the-vision's channel-length flag carried only by Channel-fit's "sized"), the two defending-in-depth near-negation flags kept under the recorded security KEEP exemption (hardcoded-algorithm clear, version-prefix marginal), and one pre-existing gap outside this diff: orchestrating-products attributes 1-billion-scale statelessness to dmj:enforcing-performance-budgets, which never mentions it, while battery 5C's PASS requires naming that defect.
- Invocation traps recorded for future gate scripts: rename-check.js silently reports PURE on empty piped stdin (run it from a file); behavioral-test.sh grades only the choice letter, so a green run is a spot-check, not battery coverage.
- Recertification debt unchanged: descriptions frozen byte-identical; full routing-gauntlet and tier-degradation reruns stay release-gated per 2.19.0; six orchestration files carry no battery section, so a tier-degradation rerun is their only behavioral proof.
